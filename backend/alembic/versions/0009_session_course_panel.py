...
Revision ID: 0009_session_course_panel
Revises: 0008_courses_panels
Create Date: 2026-07-18 00:00:01.000000

"Extending the attendance system" spec, Part 5/6 — Session Creation
Workflow + Attendance Filtering. Ties an attendance session to the
specific `Course` and `Panel` a teacher selected when starting it. Purely
additive:
...
existing rows simply get NULL in both new columns, per "do not lose
existing attendance records." Going forward, the teacher "Start
Attendance" flow requires choosing both (enforced in
`AttendanceSessionService.start_session` and `AttendanceSessionStartRequest`),
but that is an application-layer rule, not a NOT NULL constraint here, for
the same reason `students.panel_id` (migration 0008) isn't one either.
...
revision: str = "0009_session_course_panel"
down_revision: Union[str, None] = "0008_courses_panels"
